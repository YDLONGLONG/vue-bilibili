<template>
  <div>
    <!-- 顶部 -->
    <div class="topBg">
      <el-select v-model="value" placeholder="视角：我自己">
      <el-option value="视角：我自己" />
      <router-link :to="`/user/${user._id}`"><el-option value="视角：新访客" ></el-option></router-link>
      </el-select>
    ...
    <div class="middle">
      <el-row>
        ...
        <el-col :span="2" :offset="6" style="line-height: 200%;font-size: 1.4rem;">
          <p>粉丝数</p>
...
import { getUserById, updateUser, getAttentionUser } from '../../api/user'
export default {
  data() {
    return {
      user: {},
  ...
  methods: {
    ...
    async init() {
      ...
      this.user = result.data
      let resulted = await getAttentionUser(this.userId)
      this.attentions = resulted.data
    }
  },
  created() {
    this.init()
